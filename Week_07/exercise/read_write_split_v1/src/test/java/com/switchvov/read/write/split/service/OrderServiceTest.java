...
public class OrderServiceTest {
    ...
    public void routingSaveOrGet() {
        for (int i = 0; i < 10; i++) {
            ...
            log.info("保存是否成功:{}", saved);
            // 因为主从同步有延时，故这次查询一般返回为null
            Order selectOrder1 = orderService.routingGetByOrderId(order.getOrderId());
            log.info("从数据库中获取的order:{}", selectOrder1);
            // 1s后在查询
            TimeUnit.SECONDS.sleep(1);
            Order selectOrder2 = orderService.routingGetByOrderId(order.getOrderId());
            log.info("从数据库中获取的order:{}", selectOrder2);
            log.info("\n\n");
        }
    }
